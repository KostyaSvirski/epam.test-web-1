...
<html lang="en">
  ...
  <body>
    ...
    <main class="main">
        <h1  class="main_text"><fmt:message key="penalty.penalty"/></h1>
        <div class="main">
        <form class="decor" action="MainController" method="POST">
            ...
            <c:if test="${sessionScope.user.roleInProject == 'ADMIN'}">
	            ...
	            <div class="radio">
	                <input type="radio" name="problems" value="YES"><fmt:message key="release.problems.yes"/>
	                <input type="radio" name="problems" value="NO"><fmt:message key="release.problems.no"/><br>
	            </div>
	            <input type="text" placeholder="<fmt:message key="release.problems.text"/>" name="info">
	            <input type="text" placeholder="<fmt:message key="release.problems.cost"/>" name="penalty">
	            <input type="submit" value = "<fmt:message key="orders.button.confirm"/>"/>            
            </c:if>
            <c:if test="${sessionScope.user.roleInProject != 'ADMIN'}">
            	<input type="hidden" name="command" value="RELEASE_ORDER_COMMAND">
	            <input type="hidden" name="order_id" value="${param.id}">
	            <input type="text" placeholder="<fmt:message key="release.comment"/>" name="info">
	            <input type="submit" value = "<fmt:message key="orders.button.confirm"/>">
            </c:if>
        </div>
        </form>
        </div>
        <a href = "my_page.jsp" class="register"> <fmt:message key="cars.button.back"/> </a>
    </main>
