    ...
    <section class="inf_about_car">
      <section class="info">
        <div class="text" id="text-id">
          <p align="center"><b><fmt:message key="penalty.penalty"/></b></p>
	      <form action="MainController" method="GET">
	          <input type="hidden" name="command" value="CLOSE_PENALTY_COMMAND">
	          <input type="hidden" name="id" value="${penalty.id}">
	          <p style="text-align: center">${penalty.info}</p>
	          <p style="text-align: center"><fmt:message key="cars.info.cost"/> ${penalty.costOfPenalty} $
	          <c:if test="${penalty.isClosed == false}">
	          	<p style="collor: red; text-align: center"><fmt:message key="penalty.info.not_closed"/></p>
	          </c:if>
	          <c:if test="${penalty.isClosed == true}">
	          	<p style="collor: green; text-align: center"><fmt:message key="penalty.info.closed"/></p>
	          </c:if>
	          <div class="all_button">
	          <c:if test="${penalty.isClosed == false}">
	          	<c:if test="${sessionScope.user.roleInProject != 'ADMIN'}">
		            <input type = "submit" id="button-form" value="<fmt:message key="penalty.button.close"/>">	          		
	          	</c:if>
	          </c:if>
	          </div>
